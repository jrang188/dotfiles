...
- **`1password-cli` vendored at 2.38.1** in `nix/pkgs/1password-cli.nix`, consumed via `pkgs.callPackage` in `nix/home/packages.nix`. nixpkgs (unstable + 26.05) lags at 2.34.x and the only open bump PR ([NixOS/nixpkgs#542758](https://github.com/NixOS/nixpkgs/pull/542758)) is a draft to a beta (2.38.2-beta.01), so no stable 2.35+ is imminent. The vendored file is a copy of `pkgs/by-name/_1/_1password-cli/package.nix` trimmed to the two active platforms (`x86_64-linux`, `aarch64-darwin`) with `version` + SRI hashes bumped. Bump by running `nix store prefetch-file --json` on the matching `op_<platform>_v<version>.{zip,pkg}` URLs from [app-updates.agilebits.com/product_history/CLI2](https://app-updates.agilebits.com/product_history/CLI2). **Drop the vendored package and revert to `pkgs._1password-cli` once nixpkgs ships >= 2.38.1.**
