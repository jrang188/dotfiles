...
- **Hyprland pinned to v0.56.0** on `kirby` via direct flake input `inputs.hyprland` (not an overlay), with `hy3` at `hl0.56.0.1` ABI-locked via `follows`. Bump both together. nixpkgs' `hyprland` lags behind (still 0.54.3) and `hyprlandPlugins.hy3` in nixpkgs no longer matches it, which is why the direct upstream pin is required for ABI alignment.
- **hyprlang config deprecation (FOLLOW-UP NEEDED):** as of Hyprland 0.55 the `.conf` config format is deprecated in favor of `hyprland.lua`. The repo still uses `configType = "hyprlang"` in `home/nixos/kirby/hyprland.nix:7` and is on `home.stateVersion = "24.11"` (which defaults to hyprlang). Works fine on 0.56 because home-manager only writes `hyprland.conf` (never both files), so Hyprland loads it as hyprlang. The maintainer stated hyprlang will be supported for "1–2 releases starting from 0.55" — removal expected in 0.57 or 0.58. **Before the next Hyprland major bump, migrate to Lua**: change `configType` to `"lua"` and rewrite `home/nixos/kirby/hyprland.nix` (settings table + `extraConfig` `plugin { hy3 { } }` block) to Lua. Also bump `home.stateVersion` to `26.05` in `home/nixos/kirby/default.nix` so home-manager's default flips to `"lua"`. Tracked as a separate todo.
