...
- **`Sterling-MBP`** — macOS (Darwin), aarch64. Uses Determinate Nix (`nix.enable = false`), `mac-app-util` pinned independently, nix-homebrew, Home Manager (darwin branch).
...
- **Known pinning / overrides**: `mac-app-util` does NOT follow nixpkgs (SBCL 2.6.0 build failure); Hyprland pinned for `hy3` ABI compatibility; `lanzaboote` pinned to v1.1.0; Darwin `pre-commit` pulled from `pkgs-stable` (dotnet dependency issue).
...
| `nix/modules/darwin/` | Darwin modules: pre-commit overlay from `pkgs-stable`, system defaults, dock, Touch ID (PAM). Determinate Nix convention. | [View Map](nix/modules/darwin/codemap.md) |
