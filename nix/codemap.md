...
- **External flake pinning for stability**: `mac-app-util` follows `nixpkgs-darwin` (the previously independent pinning was removed 2026-07). `hy3` is pinned to upstream `hl0.56.0.1` because nixpkgs' `hyprlandPlugins.hy3` lags at 0.55.0 (one minor behind the matching Hyprland 0.56.0 ABI); nixpkgs' `hyprland` (0.56.0) is used directly. `lanzaboote` is pinned to v1.1.0.
