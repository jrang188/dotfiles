...
- **Tmux integration**: `tmux.nix` configures `programs.tmux` with `C-Space` prefix, vi mode, 1-based base index, mouse, focus events, `tmux-256color` with RGB/undercurl terminal-features for Ghostty, cwd-preserving splits (`|`/`-`/`%`/`"`) and new windows (`c`), `Tab` last-window, prefix `h/j/k/l` pane selection with repeatable `H/J/K/L` resize, and the `vim-tmux-navigator` (prefix-free `C-h/j/k/l` nvim↔pane nav), `yank`, and `tokyo-night-tmux` tmux plugins. `zsh.nix` enables the `"tmux"` oh-my-zsh completion plugin for shell-level tmux integration.
