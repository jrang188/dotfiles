...
- **Flat module composition**: Each concern (zsh, neovim, git, etc.) lives in its own top-level `.nix` file, imported explicitly by `default.nix`. No subdirectory nesting except `./ai`.
...
3. `nix/home/default.nix` is evaluated first — it imports submodules: `zsh.nix`, `neovim.nix`, `yazi.nix`, `git.nix`, `oh-my-posh.nix`, `packages.nix`, and `./ai`.
