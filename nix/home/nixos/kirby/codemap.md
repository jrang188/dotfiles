...
- **Hyprland as primary WM**: `hyprland.nix` (393 lines) is the largest module — full compositor config with hy3 tiling plugin, custom keybindings (Alt-based navigation inspired by Aerospace), hy3 workspace semantics, multimonitor support, multimedia keys, hyprlock (lock screen), hypridle (idle management), hyprpaper (wallpaper), and hyprsunset (blue light filter).
...
1. `flake.nix` → `mkSystem` for host `kirby` → `homeImports = [ ./home ./home/nixos/kirby ]` with `extraArgs` passing `zen-browser`, `llm-agents`, `hyprland`, and `hy3` inputs.
...
4. Each submodule sets host-specific options. The `hy3` plugin is loaded as a Hyprland plugin (line 227–229 of `hyprland.nix`).
...
- **External flakes**: `zen-browser` (github:0xc000022070/zen-browser-flake), `hy3` (github:outfoxxed/hy3), `hyprland` (github:hyprwm/Hyprland).
